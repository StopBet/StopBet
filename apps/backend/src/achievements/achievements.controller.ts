import { Body, Controller, Get, HttpCode, NotFoundException, Param, ParseIntPipe, Post, UseGuards } from '@nestjs/common';
...
export class AchievementsController {
  ...
  getAchievements(@UserId() userId: string) {
  ...
  // El psicólogo registra la recaída desde la ficha del paciente. Antes usaba el endpoint de
  // arriba mandando el id del PACIENTE en x-user-id, es decir, haciéndose pasar por él.
  @Post('patients/:patientId/relapse')
  @UseGuards(RolesGuard, PatientAccessGuard)
  @Roles('psychologist', 'coordinator')
  @HttpCode(200)
  @ApiOperation({ summary: 'El equipo clínico registra una recaída de un paciente' })
  ...
  reportPatientRelapse(@Param('patientId') patientId: string) {
    return this.achievementsService.reportRelapse(patientId);
  }
