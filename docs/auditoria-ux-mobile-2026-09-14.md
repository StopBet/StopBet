...
- [x] **SIS-07** · Sin modo oscuro. **Arreglo:** tema oscuro por tokens. _(cada dueño en su pantalla)_ — **Resuelto en local 15-09.** `StyleSheet.create` corre una sola vez al cargar el módulo, así que una hoja fija no puede cambiar de tema: cada archivo declara ahora `makeStyles(c)` y cada componente la resuelve con `useStyles`, que memoriza una hoja por tema. Se migraron **893 referencias de color en 32 archivos y 58 componentes**.
  - **El manual de marca no se tocó.** El azul, el verde y el rojo siguen siendo los mismos como **relleno**: un botón azul con texto blanco da 5,09:1 en los dos temas. Lo que se separó son los tokens de **texto**, porque `#396fb6` sobre fondo oscuro da 3,16:1 y `#B83232` da 2,72:1. Ahora hay `primaryText` y `dangerText` —claros en oscuro, de marca en claro—, que es el mismo criterio que ya se usaba con `greenText`.
  - **Medido, no estimado:** los 12 pares de contraste principales dan **cero por debajo de 4,5:1 en los dos temas** (el peor en oscuro es 4,57:1, el mismo que en claro). Ni negro puro ni gris plano de fondo: el negro puro con texto claro deja halos en pantallas OLED.
  - La barra de estado sigue al tema; los diálogos nativos se quedan claros a propósito (ver AND-02).
  - **Se elige en Perfil → Apariencia**, con tres opciones: *Automático* (sigue al teléfono, es lo que viene por omisión), *Claro* y *Oscuro*. No alcanzaba con seguir al sistema: alguien puede tener el teléfono en claro y querer la app oscura para el check-in de la noche. La elección se guarda en `AsyncStorage` y sobrevive al reinicio.
  - Verificado en emulador pantalla por pantalla en los dos temas, incluyendo que el claro quedó idéntico a antes.
...
- [x] **INI-05** · «Ver todo» de notificaciones no hace nada. **Arreglo:** lista vertical de las no leídas y cada una lleva a su pantalla. _(check-in HdU07 · Matías Barraza)_ — **Mitigado en local 15-09:** el carrusel con puntitos pasó a lista vertical —las notificaciones 2 y 3 ya no quedan escondidas— y «Ver todo», que no llevaba a ninguna parte, se reemplazó por el contador «N sin leer». **Falta que cada una lleve a su pantalla:** `Notification` solo trae `type` (`info`/`danger`/…), sin destino, así que enrutarlas necesita un campo nuevo en el backend. Inventar el destino a partir del tipo sería adivinar. **Cerrado el 22-09:** la lista salió del Inicio a una pantalla propia, a la que se llega por una campana con contador en el encabezado —dentro del Inicio no tenía techo y con seis avisos empujaba la racha, el check-in y el asistente fuera de pantalla—, y `Notification` ganó el campo `target` que faltaba, así que tocar una abre la pantalla que le corresponde. En el Inicio se quedan solo las `danger`, porque una alerta de pánico no puede estar a un toque de distancia.
...
- [x] **SIS-13** · Identidad de demo fija en la interfaz. **Arreglo:** Ver detalle. _(cada dueño en su pantalla)_ — **Resuelto en local 15-09, y era más que el nombre.** El login tenía un `TODO`: esperaba 900 ms y entraba. **Cualquier correo con cualquier clave abría la sesión**, y las siete pantallas leían un `TEMP_USER_ID` fijo, así que toda cuenta mostraba el progreso de Carlos Demo. Ahora `LoginScreen` llama a `POST /auth/login`, el token va en `Authorization: Bearer` con rotación automática ante un 401, y el id sale de la sesión. Verificado en emulador con tres pacientes: Ana (día 79), Pedro (día 13) y Jorge (día 0), cada uno con sus notificaciones y su check-in. **Dos fugas entre cuentas que solo aparecieron al haber sesiones reales y que van en el mismo cambio:** las cachés sin conexión usaban una clave única para toda la app —entrar con otra cuenta y quedarse sin red mostraba el progreso y el teléfono del padrino del paciente anterior— y el detector de recaída externa guardaba el número de intento del usuario previo, así que al cambiar de cuenta le anunciaba a quien recién entraba «tu psicólogo registró una recaída en tu historial».
