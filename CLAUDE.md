...
> _Actualizado 2026-09-25. Mantener al día tras cambios significativos (ver [Trabajando con Claude Code](#trabajando-con-claude-code))._

- **Mobile — dos apps en un mismo binario.** `App.tsx` enruta **por rol** tras el login: `patient` va a `AppNavigator` y `psychologist` a `StaffTabs` (Resumen · Comunidad · Perfil). El `coordinator` **no entra**, y no por producto: `GET /psychologists/:id` filtra por `role: 'psychologist'` y le da 404, y `assertPsychologist` le cierra la moderación. La vista del equipo clínico es un espejo condensado del Resumen de la web, **de solo lectura**, salvo publicar anuncios, **leer y responder el chat de la sede** (pestaña *Chat*, decisión del PO del 22-09: se ve con **las mismas burbujas que el paciente**, `components/ChatMessage.tsx`, y se responde con toque largo o «···», que abre la pantalla del stack `StaffThread`, no un composer dentro de la pestaña, por el pager y el teclado) y borrar publicaciones reportadas; **no monta pánico, asistente ni check-in, y su barra no lleva el botón SOS** (crearía una alerta a nombre del psicólogo).
...
- **La insignia avisa sola (25-09, HdU3 CA1):** las insignias nacían dentro de `getAchievements()`, o sea al entrar a Logros: con la app cerrada no se otorgaban y no había qué notificar, que es justo lo que pide el criterio. El otorgamiento quedó centralizado en `otorgarInsigniasPendientes()` —lo llaman la pantalla, el endpoint de dev y `BadgeNotifierService`, un cron a las **09:00 de Chile** sobre los períodos abiertos— y la fila de `earned_badges` es el punto de deduplicación, así que el aviso sale **una sola vez**, lo gane el cron o la pantalla. La notificación se guarda con `target: 'achievements'` antes de mandar el push, y **`reportRelapse` no felicita**: ahí se cierran los hitos alcanzados antes de la recaída. Si se otorgan varios hitos de golpe (recuperar días atrasados) se avisa solo por el más alto.
...
- ⚠️ **Las escrituras contra Railway se pierden de vuelta (02/03-09). Antes de "arreglarlo" otra vez, lee esto.** El proxy de Railway mata las conexiones inactivas a los 60 s con un RST y no le avisa al cliente (medido: sobrevive a 58 s, muere a 60 s). El teléfono reutiliza una conexión ya muerta, el servidor **recibe y procesa el POST**, pero la respuesta no vuelve: la app muestra `TypeError: Network request failed` con el dato ya guardado. Solo rompe escrituras, porque OkHttp reintenta los GET por su cuenta y un POST nunca. Está atacado por cuatro lados y **los cuatro siguen siendo necesarios**:
  - `MainApplication.kt` baja el pool de OkHttp de 5 minutos a 30 s, para que no exista una conexión zombi que reutilizar, y **fuerza HTTP/1.1** (`Protocol.HTTP_1_1`): Railway negocia h2 por ALPN y sobre HTTP/2 la escritura completa bien en OkHttp —201/409, `callEnd` OK— pero la respuesta nunca llega a JS. Es la única diferencia estructural entre "en local funciona" y "en Railway no".
...
- **Mobile** (React Native CLI 0.86): compila y corre en Android físico y en emulador. Flujo y *gotchas* del monorepo en `apps/mobile/README.md`. El check-in se encola en `AsyncStorage` si no hay red y se reintenta al reconectar; el asistente muestra una tarjeta de crisis (pánico / compañero de viaje / `*4141`) ante riesgo alto, y un mensaje de respaldo dentro del hilo si el envío falla.
  - **Rendimiento (22-09, `docs/auditoria-rendimiento-mobile-2026-09-22.md`): 15 de 17 hallazgos aplicados.** El bundle de producción pasó de **3,29 MB a 1,78 MB**. Lo que hay que respetar al escribir código:
    ...
    - **Las filas de una lista van memorizadas y las listas que crecen van en `FlatList`.** `ChatMessage` (`components/ChatMessage.tsx`, la burbuja del chat, compartida por paciente y equipo clínico) usa `React.memo` con un comparador que ignora los callbacks a propósito (está explicado sobre el componente): si le agregas una prop de datos, súmala al comparador.
  ...
  - **El campo RUT usa `keyboardType="visible-password"` (02-09)**, y no es un descuido: `formatRut` reescribe el string en cada tecla y el teclado predictivo de Android, al perder su región de composición, soltaba el buffer entero y dejaba `123123123`. Apagar `autoCorrect` no basta —el teclado de Samsung lo ignora—; lo que corta la duplicación es que Android no deja que el predictivo edite un campo de contraseña. `numbers-and-punctuation` tampoco servía: era solo iOS y dejaba fuera la K del dígito verificador.
...
- **Web dashboard**: vistas del terapeuta (login, Overview, Alertas, Finanzas, Solicitudes, Sesiones de familiares, Configuración) conectadas a la API real con TanStack Query (`@tanstack/react-query`). `Finanzas` y `Configuración` siguen con datos mock. El cliente HTTP (`apps/web/src/services/api.ts`) manda `Authorization: Bearer` con refresh automático; ya no manda `x-user-id`: la ficha registra recaídas y el PDF lee pagos por endpoints del equipo clínico acotados por asignación.
  ...
  - **Cerrar sesión limpia la caché de TanStack Query** (`queryClient.clear()` en `App.tsx`, 02-09): las claves no llevan el id del usuario, así que la cuenta siguiente heredaba pacientes, alertas, solicitudes y sesiones de la anterior, y con `staleTime: 30_000` ni siquiera se volvían a pedir. Recargar la página lo tapaba, porque la caché es solo de memoria.
...
- **DB local**: datos de prueba en PostgreSQL (usuario `postgres`, pass `password`, db `stopbet`) creados con `pnpm run seed` — 9 usuarios (patient/sponsor/psychologist/coordinator), todos con la misma clave de desarrollo `Stopbet2026!`. El script viejo `python scripts/populate_db.py` sigue existiendo pero `pnpm run seed` es la vía actual.
  - **`pnpm run seed:demo -- --reset`** (03-09) puebla lo que los otros seeds no tocan nunca —psicólogos con varias sedes, pacientes asignados, solicitudes pendientes, notificaciones, suscripciones y facturas— y refresca lo que caduca cada 24 h: las alertas de pánico y el check-in "de hoy". Por eso conviene correrlo **la misma mañana** de una demo. Es idempotente, como los otros dos. Runbook y cuentas en [`docs/demo-sprint1.md`](docs/demo-sprint1.md).
...
- **Portal del familiar (HdU11)**: módulo `family` con vínculo familiar↔paciente, sesiones grupales por sede y confirmación de asistencia. Dos avisos:
  ...
  - **Las sesiones de demo se mantienen solas (25-09):** al arrancar y cada día a las 05:00 de Chile, `DemoService` crea las que falten y mueve hacia adelante las que ya pasaron (`family/family-demo-sessions.ts`, que también usa el seed). Viene encendido y se apaga con `DEMO_SESIONES_FAMILIARES=false`; sin Carlos Demo en la base no hace nada. Existe porque la base de Railway se sembró una sola vez y el portal se quedaba sin sesiones próximas.
- **Deudas técnicas**:
...
pnpm run seed:demo -- --reset  # datos de demo: asignaciones, solicitudes, cobros y lo que caduca cada 24 h
