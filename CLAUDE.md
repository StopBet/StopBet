...
- **Deudas técnicas**:
  ...
  - ⚠️ **La base de datos de Railway está vacía.** El backend y el Postgres de producción funcionan (`/health` responde `200`), pero nadie ha corrido `pnpm run seed` ni `pnpm run seed:family` contra ella todavía — sin eso no hay ninguna cuenta con la cual entrar en la demo. Procedimiento en `README.md` → "Despliegue en producción".
